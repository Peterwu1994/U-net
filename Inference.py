...
checkpoint_file = '/raid/wuyudong/Models/DeeplabGuidewire/Mobilenet/Train/Atrous16/checkpoint'
...
rst_file = '/raid/wuyudong/Models/DeeplabGuidewire/Mobilenet/Inference/Atrous16.pickle'
        ...
        def SessRun(ckpt_path):
            ...
            with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, log_device_placement=False)) as sess:
                ...
                for i in tqdm(range(num_loops)):
                    ls, ps = sess.run([labels, pred])
                    ls = ls.astype(np.uint8)
                    ps = ps.astype(np.uint8)
                    ls = ls * 255
                    ps = ps * 255
                    # im = ls + ps
                    # io.imsave(os.path.join('/home/wuyudong/Project/ImageData/Temp/L', '%d.png' % i), np.squeeze(im[0]))
                    # metric
                    dist_l_r, dist_r_l = DistancesFromLeftToRightsMultis(ps, ls, skeletonize=False)
